var contentData = 
{
	"vertices-and-edges": {
		...
		"theory-content":		"<div><span>A graph is a collection of vertices<\/span><\/div><svg width=30 height=30 xml:space='preserve' style='margin-left: 0; margin-right: 0'><g><circle cx='20' cy='20' r='10' fill='steelblue'/></g> </svg> <div><p>interconnected by edges. In the figure, vertices are represented by circles and edges by lines. We denote the set of all vertices by \\(V\\) and set of all edges by \\(E\\).<\/p><p>Got it? Now in easy words: A graph has two components - a <b>set of vertices<\/b> \\(V\\) AND a <b>set of edges<\/b> \\(E\\). Where an edge is something acting as a link between two vertices. Period.<\/p><p>If an edge connects two vertices \\(v_1\\) and \\(v_2\\), then we denote the edge by \\(v_1v_2\\), which is same as \\(v_2v_1\\).<\/p><p>Two vertices are said to be <b>adjacent<\/b> if they are connected by an edge.<\/p><p>Now read the instructions below and create your own graph. You will see the corresponding sets \\(V\\) and \\(E\\) below the graph area. Start playing around and get yourself comfortable with the set notations.<\/p><\/div>",
